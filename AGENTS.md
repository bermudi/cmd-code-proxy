...
- **`x-session-id` is a header, separate from `threadId` (body).** Different formats too: `sess_` + 16 hex chars vs full UUID. The pi extension generates one per pi session and sends it as `x_command_code_session_id`; the proxy generates a per-request fallback if absent.
