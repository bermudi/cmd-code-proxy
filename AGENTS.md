...
- **`x-taste-learning` should reflect the user's actual `command-code` preference, not be hardcoded to `"true"`.** The real binary reads `userConfig.tasteLearning` (default `true`) and sends `"true"`/`"false"` accordingly. The proxy currently lies about this — for the user's install (taste learning **off**), the proxy still sends `"true"`. See MAINTAINING.md § Taste learning for the wire-format analysis. The pi extension should forward the user's `tasteLearning` config as `x_command_code_taste_learning`; a `-taste-learning` CLI flag is a manual override.
